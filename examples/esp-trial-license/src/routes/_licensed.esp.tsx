...
export const Route = createFileRoute("/_licensed/esp")({
  ...
  return (
    <div className="flex h-screen flex-col antialiased">
      ...
      <div className="custom-scroll-bar w-full flex-1 overflow-auto px-6 pt-6">
        {["effort", "success", "progress"].map((espType) => (
          <ESPSection
            key={`${espType}-${date}`}
            date={date}
            type={espType as ESPType}
...
type ChooseImageButtonProps = {
  onImageChosen: (file: string) => void;
};

function ChooseImageButton({ onImageChosen }: ChooseImageButtonProps) {
